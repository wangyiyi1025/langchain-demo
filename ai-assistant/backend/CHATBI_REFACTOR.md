...
本次重构采用**职责分离 + 预定义链 + 原子工具 + 上下文传递**的设计模式，解决了之前架构中的以下问题：
...
6. **性能优化**：预定义工具链减少70%的LLM调用次数，大幅降低延迟
...
┌──────────────────────────┐  ┌──────────────────┐
│ 预定义工具链              │  │ 原子工具层        │
│ (chatbi_chains.py)       │  │ (atomic.py)      │
├──────────────────────────┤  ├──────────────────┤
│ ⚡️ query_only_chain     │  │ 🔧 get_schema    │
│   LLM: 2次               │  │ 🔧 nl_to_sql ⭐️ │
│                          │  │ 🔧 execute_sql   │
│ ⚡️ query_with_analysis  │  │ 🔧 analyze_data  │
│   LLM: 3次               │  │ 🔧 suggest_chart │
│                          │  │ 🔧 generate_cfg  │
│ ⚡️ query_with_chart     │  └──────────────────┘
│   LLM: 3次               │
│                          │
│ ⚡️ full_analysis_chain  │
│   LLM: 4次               │
└──────────────────────────┘
```
...
**问题**：原子工具虽然灵活，但存在性能问题：
- 每个工具调用之间，Agent都要LLM思考一次
- 简单的4步查询流程，需要5-6次LLM调用
- 累积延迟明显（每次1-3秒）
...
#### 1. chatbi_query_only_chain - 只查询 ⭐️ 最快
...
**流程**：
1. get_current_time（无LLM）
2. get_schema_info（无LLM）
3. nl_to_sql（1次LLM）
4. execute_sql（无LLM）
...
**性能**：
- LLM调用：2次（Agent决策1次 + nl_to_sql内部1次）
- 相比原子工具：从6次 → 2次，减少70%
- 延迟：约2秒

#### 2. chatbi_query_with_analysis_chain - 查询+分析
...
**性能**：
- LLM调用：3次（Agent 1次 + nl_to_sql 1次 + analyze_data 1次）
- 相比原子工具：从7次 → 3次，减少60%
- 延迟：约3秒

#### 3. chatbi_query_with_chart_chain - 查询+可视化
...
3. nl_to_sql（1次LLM）
4. execute_sql（无LLM）
5. suggest_chart（1次LLM）
6. generate_chart_config（无LLM）
...
**性能**：
- LLM调用：3次（Agent 1次 + nl_to_sql 1次 + suggest_chart 1次）
- 相比原子工具：从7次 → 3次，减少60%
- 延迟：约3秒

#### 4. chatbi_full_analysis_chain - 完整分析
...
4. execute_sql（无LLM）
5. analyze_data（1次LLM）
6. suggest_chart（1次LLM）
7. generate_chart_config（无LLM）
...
| 场景 | 原子工具 | 工具链 | 性能提升 |
|------|---------|--------|----------|
| 只查询 | 6次LLM，~6秒 | 2次LLM，~2秒 | ⬆️ 70% |
| 查询+分析 | 7次LLM，~7秒 | 3次LLM，~3秒 | ⬆️ 60% |
| 查询+可视化 | 7次LLM，~7秒 | 3次LLM，~3秒 | ⬆️ 60% |
| 完整分析 | 11次LLM，~11秒 | 4次LLM，~4秒 | ⬆️ 65% |
